...
import AddFormCreateUser from "../component/create_user/AddFormCreateUser";
const CreateUser = () => {
  const menus = [
  ...
  return (
    <section className="flex gap-6 bg-gray-900">
      <div
        ...
        <div className="mt-4 flex flex-col gap-4 relative">
          {menus?.map((menu, i) => ( <Link to={menu?.link} key={i} className={` ${menu?.margin && "mt-5"} group flex items-center text-[12px]  gap-3.5 font-medium p-2 hover:bg-gray-800 rounded-md sm:text-sm`}>
              <div>
      ...
      <div className="m-3 text-[12px] text-gray-900 font-semibold md:text-sm justify-center bg-gray-900">
        <AddFormCreateUser/>
      </div>
    </section>
  );
